"""
Document processing functionality for AIFluent
...
from aifluent.base import DocStatus
from aifluent.parser import MineruParser, MineruExecutionError, get_parser
from aifluent.utils import (
    separate_content,
    insert_text_content,
...
class ProcessorMixin:
    """ProcessorMixin class containing document processing functionality for AIFluent"""
    ...
    def _apply_chunk_template(
        ...
        """
        from aifluent.prompt import PROMPTS
        ...
        try:
            ...
            if pipeline_status_lock and pipeline_status:
                try:
                    async with pipeline_status_lock:
                        ...
                        error_msg = (
                            f"AIFluent processing failed for {file_name}: {str(e)}"
                        )
            ...
            async with pipeline_status_lock:
                ...
                pipeline_status["latest_message"] = (
                    f"AIFluent processing completed for {file_name}"
                )
                pipeline_status["history_messages"].append(
                    f"AIFluent processing completed for {file_name}"
                )
